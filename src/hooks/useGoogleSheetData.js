...
// This needs to be refactored for cleaner async fetching. Maybe bundle everything using Promise.all()
  ...
  useEffect(() => {
    const fetchSheetData = async () => {
      try {
        ...
        setError(error.message) // Here we're capturing the error message
      }
    }

    fetchSheetData()
  }, []) //  only need to run once on page render
